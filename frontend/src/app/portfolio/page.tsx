...
const PortfolioGlobe = dynamic(
  () => import("@/components/three/PortfolioGlobe"),
  { ssr: false },
);
...
export default function PortfolioPage() {
  ...
  if (latest) {
    ["Equity", "Oil", "Gold", "Bond"].forEach((asset) => {
      ...
      const assetPnl =
        initialPrice > 0 ? (price - initialPrice) / initialPrice : 0;
              ...
              <tbody>
                {assets.map((a) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 number">
                      {(a.weight * 100).toFixed(1)}%
                    </td>
                    <td
                      className={`px-4 py-3 number ${a.pnl >= 0 ? "text-[var(--gain)]" : "text-[var(--loss)]"}`}
                    >
                      {formatPercent(a.pnl)}
                    </td>
