...
const RiskSphere = dynamic(() => import('@/components/three/RiskSphere'), { ssr: false });
const MarketParticles = dynamic(() => import('@/components/three/MarketParticles'), { ssr: false });
...
const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number }>; label?: string }) => {
  ...
  return (
    <div className="glass-card p-3 text-xs" style={{ minWidth: 160 }}>
      ...
      <div className="number text-white">{formatCurrency(payload[0]?.value)}</div>
    </div>
  );
};
...
export default function DashboardPage() {
  ...
  const varValue = metrics ? Math.abs(metrics['Max Drawdown'] ?? 0.02) * 0.1 : 0.023;
  ...
  return (
    <>
      <MarketParticles />
      <motion.div {...pageTransition} className="p-6 space-y-6 max-w-7xl">
        ...
        <div className="flex items-start justify-between gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <span className={`text-sm font-medium number ${totalReturn >= 0 ? 'text-[var(--gain)]' : 'text-[var(--loss)]'}`}>
              {totalReturn >= 0 ? '+' : ''}{formatCurrency(latestValue - firstValue)} ({formatPercent(totalReturn)})
            </span>
              ...
              {metrics && (
                <>
                  <div>
                    ...
                    <div className="number text-white text-lg">{(metrics['Sharpe Ratio'] ?? 0).toFixed(2)}</div>
                  </div>
                  <div>
                    <div className="label">VaR (est.)</div>
                    <div className="number text-[var(--warn)] text-lg">{(varValue * 100).toFixed(1)}%</div>
                  </div>
                  <div>
                    <div className="label">Max DD</div>
                    <div className="number text-[var(--loss)] text-lg">{formatPercent(metrics['Max Drawdown'] ?? 0)}</div>
                  </div>
                </>
              )}
            </div>
        ...
        <GlassCard className="p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-white font-medium text-base">Portfolio Value Over Time</h2>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={chartData}>
              ...
              <Area
                type="monotone" dataKey="value" stroke="#00D4AA" strokeWidth={2}
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <GlassCard className="p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(52,199,89,0.12)' }}>
          ...
          <GlassCard className="p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(255,159,10,0.12)' }}>
