...
export async function GET() {
  try {
    const filePath = path.join(process.cwd(), 'public', 'data', 'trade_log.jsonl');
    if (!fs.existsSync(filePath)) {
      const mock = Array.from({ length: 15 }, (_, i) => ({
        ...
        costs: { commission: (Math.random() * 50).toFixed(2), slippage: (Math.random() * 25).toFixed(2) },
        portfolio_state: { cash: (50000 + Math.random() * 10000).toFixed(2), total_value: (1000000 + Math.random() * 50000).toFixed(2) }
      }));
